...
const icons = [
  { id: uniqueId(), icon: <HeartIcon /> },
  { id: uniqueId(), icon: <ChatCircleIcon /> },
  { id: uniqueId(), icon: <PaperPlaneTiltIcon /> },
];

const InstagramPreviewCard = ({ styles }) => {
  return (
    <div className={styles.previewCardWrap}>
      <div className={styles.centeredWithGap}>
        <img src={avatar} alt="avatar" className={styles.avatar} />
        <div className={`${styles.name} ${styles.instagramCatdName}`}>
          John Doe<span>9h</span>
        </div>
      </div>
      <img src={media} alt="post-img" className={styles.postImg} />
      <div className={styles.centeredWithSmallGap}>
        {icons.map(({ id, icon }) => (
          <div key={id} className={styles.iconWrap}>
            {icon}
          </div>
        ))}
      </div>
      {/* <div style={{ color: "#000", fontSize: "10px", fontWeight: 700  }}>
        likes
      </div> */}
      <div className={`${styles.postContent} ${styles.instagramPostContent}`}>
        {postContent}
      </div>
      <div className={styles.secondaryText}>Add a comment...</div>
    </div>
  );
};
