...
const footerIcons = [
  ...
  { id: uniqueId(), title: "Comment", icon: <ChatCircleIcon /> },
  { id: uniqueId(), title: "Repost", icon: <RepeatIcon /> },

  { id: uniqueId(), title: "Share", icon: <ShareFatIcon /> },
];

const LinkedinPreviewCard = ({ styles }) => {
  return (
    <div className={styles.previewCardWrap}>
      <div className={styles.centeredWithGap}>
        <img src={avatar} alt="avatar" className={styles.avatar} />
        <div>
          <div className={`${styles.name} ${styles.linkedinCardName}`}>
            John Doe<span>1st</span>
          </div>
          <div className={styles.centeredWithGap}>
            <div className={styles.createdDate}>9h</div>
